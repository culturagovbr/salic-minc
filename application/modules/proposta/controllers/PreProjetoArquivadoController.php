    ...
    public function avaliarArquivamentoAction()
    {
        ...
        if ($avaliacaoFinal) {
            ...
            if ($Avaliacao == null && $stDecisao == Proposta_Model_PreProjeto::ESTADO_ARQUIVADO) {
                $success = false;
                $message = "&Eacute; necessário descrever a avaliação!";
            } else {
        ...
        try {
            if($success){
                ...
                if($stDecisao == Proposta_Model_PreProjeto::ESTADO_ATIVO){
                    $corpoEmail = '<p>O pedido de desarquivamento referente &agrave; proposta supracitada foi aceito.  A proposta será desarquivada e seguirá em análise. Dessa forma, acompanhe diariamente a proposta no sistema em virtude de novas dilig&ecirc;ncias e comunicados</p>';

                } else {
                    $corpoEmail .= '<p>O pedido de desarquivamento referente &agrave; proposta supracitada não foi aceito pelo seguinte motivo:<p/></br>';
                    $corpoEmail .= "<p>{$Avaliacao}</p></br>";
                    $corpoEmail .= '<p>Salientamos que o proponente poderá inscrever e enviar novamente a mesma proposta ao MinC desde que observada a restrição contida na alínea "c", inciso I do artigo 23 da Instrução Normativa nº 05/2017 do Minist&eacute;rio da Cultura.</p></br>';
                }
