...
class spValidarApresentacaoDeProjeto extends MinC_Db_Table_Abstract
{
    ...
    public function checklistEnvioProposta($idPreProjeto)
    {
        ...
        if (!empty($movimentacao)) {
            $validacao->Descricao = '<font color=blue><b>A PROPOSTA CULTURAL ENCONTRA-SE NO MINISTÉRIO DO TURISMO.</b></font>';
            ...
            if ((date('m') == 12 || date('m') == 1) && empty($avaliacaoProposta)) {
                ...
                if (!empty($regularidadeProponente)) {
                    $validacao->Descricao ='Proponente em situação IRREGULAR no Ministério do Turismo.';
                    $validacao->Observacao =  'PENDENTE';
                    $listaValidacao[] =  clone($validacao);
                } else {
                    $validacao->Descricao ='Proponente em situação REGULAR no Ministério do Turismo.';
                    $validacao->Observacao =  'OK';
                    $listaValidacao[] =  clone($validacao);
                }
        ...
        if ($validado) {
            ...
            $validacao->Descricao = '<font color=blue><b>A PROPOSTA CULTURAL FOI ENCAMINHADA COM SUCESSO AO MINISTÉRIO DO TURISMO.</b></font>';
            $validacao->Observacao = 'OK';
            $listaValidacao[] =  clone($validacao);
        } else {
