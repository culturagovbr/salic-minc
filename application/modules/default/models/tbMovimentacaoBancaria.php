    ...
    public function buscarDados($pronac = null, $conta_rejeitada = null, $periodo = null, $operacao = null, $tamanho=-1, $inicio=-1, $count = null)
    {
        ...
        if (isset($count)) {
            ...
            $select->from(
                            array("mi" => "tbMovimentacaoBancariaItem"),
                        array("m.nrBanco"
                                    ,new Zend_Db_Expr("CONVERT(CHAR(10), m.dtInicioMovimento, 103) AS dtInicioMovimento")
                                    ,new Zend_Db_Expr("CONVERT(CHAR(10), m.dtFimMovimento, 103) AS dtFimMovimento")
                                    ...
                                    ,"mi.nmTituloRazao"
                                    ,"mi.nmAbreviado"
                                    ,new Zend_Db_Expr("CONVERT(CHAR(10), mi.dtAberturaConta, 103) AS dtAberturaConta")
                                    ,"mi.nrCNPJCPF"
                                    ...
                                    ,"mi.vlSaldoFinal"
                                    ,"mi.tpSaldoFinal"
                                    ,new Zend_Db_Expr("CONVERT(CHAR(10), mi.dtMovimento, 103) AS dtMovimento")
                                    ,"mi.cdHistorico"
                                    ,"mi.dsHistorico"
                                    ,"mi.nrDocumento"
                                    ,"mi.vlMovimento"
        ...
        if (!empty($conta_rejeitada) && $conta_rejeitada) {
            $select->where(new Zend_Db_Expr("mx.idMovimentacaoBancariaItem IS NOT NULL"));
            $select->where(new Zend_Db_Expr("mx.idTipoInconsistencia IS NOT NULL"));
        } else {
            $select->where(new Zend_Db_Expr("mx.idMovimentacaoBancariaItem IS NULL"));
        ...
        if (!empty($periodo)) {
            ...
            if ($periodo[0] == "B") { // Ontem
                $select->where(new Zend_Db_Expr("CONVERT(DATE, m.dtInicioMovimento) = DATEADD(DAY, -1, CONVERT(DATE, GETDATE()))
            ...
            if ($periodo[0] == "C") { // �ltimos 7 dias
                $select->where(new Zend_Db_Expr("CONVERT(DATE, m.dtInicioMovimento) > DATEADD(DAY, -7, CONVERT(DATE, GETDATE()))
            ...
            if ($periodo[0] == "D") { // Semana passada (seg-dom)
                $select->where(new Zend_Db_Expr("(CONVERT(DATE, m.dtInicioMovimento) >= CASE DATEPART(DW, GETDATE())
            ...
            if ($periodo[0] == "F") { // Este m�s
                $select->where(new Zend_Db_Expr("DATEPART(MONTH, m.dtInicioMovimento) + DATEPART(YEAR, m.dtInicioMovimento) = DATEPART(MONTH, GETDATE()) + DATEPART(YEAR, GETDATE())
            ...
            if ($periodo[0] == "G") { // Ano passado
                $select->where(new Zend_Db_Expr("DATEPART(YEAR, m.dtInicioMovimento) = (DATEPART(YEAR, GETDATE()) - 1)
            ...
            if ($periodo[0] == "H") { // �ltimos 12 meses
                $select->where(new Zend_Db_Expr("CONVERT(DATE, m.dtInicioMovimento) >= DATEADD(MONTH , -12, CONVERT(DATE, GETDATE()))
            ...
            if ($periodo[0] == "I") { // �ltimos 6 meses
                $select->where(new Zend_Db_Expr("CONVERT(DATE, m.dtInicioMovimento) >= DATEADD(MONTH , -6, CONVERT(DATE, GETDATE()))
            ...
            if ($periodo[0] == "J") { // �ltimos 3 meses
                $select->where(new Zend_Db_Expr("CONVERT(DATE, m.dtInicioMovimento) >= DATEADD(MONTH , -3, CONVERT(DATE, GETDATE()))
