...
class Upload
{
	...
	/**
	 * M�todo construtor que elimina o limite de tempo de execu��o
	 * @access public
	 * @param void
	 * @return void
	 */
	 ...
	 * @access public
	 * @param integer $obrig     = o envio do arquivo � obrigat�rio? (0 = nao ou 1 = sim)
	...
	{
		// tamanho m�ximo autorizado do arquivo (em bytes)
		...
		// realiza a valida��o do(s) arquivo(s) enviado(s)
		for ($i = 0; $i < count($_FILES["arquivo"]["name"]); $i++)
		{
			// pega o nome, tipo, tamanho, nome tempor�rio e erro no envio do arquivo
			...
			// dimens�es do arquivo em pixels (para imagens)
			...
			try
			{
				// verifica se o envio do arquivo � obrigat�rio
				if ($obrig == 1 && $arq_erro === UPLOAD_ERR_NO_FILE)
				{
					throw new Exception("O envio do <strong>" . ($i + 1) . "� arquivo</strong> � obrigat�rio!");
				}
				// verifica se a extens�o do arquivo faz parte das extens�es autorizadas
				...
				{
					throw new Exception("A extens�o do arquivo <strong>$arq_nome</strong> � inv�lida para upload!");
				}
				// verifica se o tamanho do arquivo � maior que o autorizado
				else if ($limit_tam == 1 && ($arq_tamanho > $tam_aut || $arq_erro === UPLOAD_ERR_INI_SIZE || $arq_erro === UPLOAD_ERR_FORM_SIZE))
				{
					throw new Exception("O arquivo <strong>$arq_nome</strong> deve ter no m�ximo " . ($tam_aut / (1024 * 1024)) . " MB!");
				}
				// largura da imagem
				...
				{
					throw new Exception("O arquivo <strong>$arq_nome</strong> j� existe!");
				}
			} // fecha try
			catch (Exception $e)
			{
	...
	public function uploadTXT($obrig, $limit_ext, $limit_tam, $limit_dim, $caminho, $sobresc)
	{
		...
		$this->uploadArquivo($obrig, $limit_ext, $limit_tam, $limit_dim, $caminho, $sobresc);
	} // fecha m�todo uploadTXT()



	/**
	 * Pega a extens�o de um arquivo
	 * @access public
	 * @static
	...
	/**
	 * Transforma o arquivo em bin�rio
	 * @access public
	 * @static
	...
	public static function setHash($arquivoTemp)
	{
        ...
        public static function cadastrarArquivosMult($files = array()) {
            ...
            for ($i = 0; $i < count($files["arquivo"]["name"]); $i++)
            {
                ...
                $arquivoTemp     = $files['arquivo']['tmp_name'][$i]; // nome tempor�rio
